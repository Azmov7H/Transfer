        ...
        <TableRow className="group border-white/5 hover:bg-white/[0.04] transition-all duration-300 cursor-default h-24">
            ...
            <TableCell className="px-8 hidden lg:table-cell">
                <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-sm font-bold text-foreground/80 group-hover:text-foreground transition-colors">
            ...
            <TableCell className="px-8 text-center">
                <div className="inline-flex flex-col items-center group/price">
                    <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-bold tabular-nums tracking-tighter text-primary">
                            {(product.retailPrice || product.sellPrice || 0).toLocaleString()}
                        </span>
                        <span className="text-xs text-muted-foreground font-bold uppercase opacity-40 group-hover/price:opacity-100 transition-opacity">ج.م</span>
                    </div>
                </div>
            </TableCell>
            ...
            <TableCell className="px-8 text-center hidden sm:table-cell">
                <div className="flex flex-col items-center gap-2">
                    <div className={cn(
                        "relative h-10 px-5 rounded-2xl border flex items-center justify-center font-bold text-lg tabular-nums shadow-inner transition-all group-hover:scale-110",
                    ...
                    <div className="flex gap-3 text-xs font-bold text-muted-foreground/30 group-hover:text-muted-foreground transition-colors uppercase tracking-widest">
            ...
            <TableCell className="px-8 text-center hidden md:table-cell">
                {stockStatus === 'out' ? (
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-destructive/10 border border-destructive/20 text-destructive text-xs font-bold uppercase tracking-widest shadow-lg shadow-destructive/5">
                        <XCircle size={12} className="animate-pulse" /> نفذت
                    </div>
                ) : stockStatus === 'low' ? (
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-warning/10 border border-warning/20 text-warning text-xs font-bold uppercase tracking-widest shadow-lg shadow-warning/5">
                        <AlertTriangle size={12} /> منخفض
                    </div>
                ) : (
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-success/10 border border-success/20 text-success text-xs font-bold uppercase tracking-widest shadow-lg shadow-success/5">
                ...
                <div className="flex items-center justify-end">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl glass-card border-white/10 shadow-2xl backdrop-blur-2xl">
                            <DropdownMenuItem onClick={() => onView(product)} className="gap-3 p-3 rounded-xl cursor-pointer transition-colors focus:bg-primary/20 focus:text-primary group/item font-bold text-sm">
                                <Eye size={18} className="text-primary opacity-60 group-hover/item:opacity-100" />
                                <span>عرض المنتج</span>
                            </DropdownMenuItem>
                            {canManage && (
                                <>
                                    <DropdownMenuItem onClick={() => onEdit(product)} className="gap-3 p-3 rounded-xl cursor-pointer transition-colors focus:bg-warning/20 focus:text-warning group/item font-bold text-sm">
                                    ...
                                    <DropdownMenuItem
                                        ...
                                        className="gap-3 p-3 rounded-xl cursor-pointer transition-colors focus:bg-destructive/20 text-destructive/60 focus:text-destructive group/item font-bold text-sm"
                                    >
                                        <Trash2 size={18} />
                                        <span>حذف نهائي</span>
                                    </DropdownMenuItem>
