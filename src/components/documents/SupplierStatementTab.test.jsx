/**
 * T-FE-DOC-014 — SupplierStatementTab unit tests.
 *
 * Locks:
...
const { SupplierStatementTab } = require('@/components/documents/SupplierStatementTab');
...
const SAMPLE = {
    ...
    supplier: { id: 's1', name: 'مورد الأمل', phone: '010', address: 'Cairo', taxNumber: 'T1' },
...
describe('SupplierStatementTab — opening balance bug fix (UI surface)', () => {
    it('shows the opening balance from the document engine (not 0)', async () => {
        render(<SupplierStatementTab supplierId="s1" />, { wrapper: makeWrapper() });
...
describe('SupplierStatementTab — line rendering', () => {
...
describe('SupplierStatementTab — wiring', () => {
    it('calls the document engine with the supplier id and date-range filters', async () => {
        ...
        expect(type).toBe('SUPPLIER_ACCOUNT_STATEMENT');
        expect(id).toBe('s1');
