            ...
            <CardHeader className="p-8 pb-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <CardTitle className="text-2xl font-bold">قائمة المواد</CardTitle>
                    ...
                    <Table aria-label="أصناف الجرد">
                        <TableHeader className="bg-muted/30">
                            <TableRow className="border-0 hover:bg-transparent">
                                <TableHead className="w-[350px] font-bold py-6 pr-8 text-primary text-right">المنتج والتفاصيل</TableHead>
                                <TableHead className="font-bold text-center">الكمية المقررة</TableHead>
                                <TableHead className="w-[200px] font-bold text-center text-primary">الكمية الفعلية (جرد)</TableHead>
                                <TableHead className="font-bold text-center">حالة المطابقة</TableHead>
                                {!isBlind && <TableHead className="font-bold text-left pl-8">الأثر المالي</TableHead>}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                    ...
                                    return (
                                        ...
                                        >
                                            <TableCell className="py-6 pr-8 text-right">
                                                <div className="flex items-center gap-4">
                                                    <div className="h-10 w-10 rounded-xl bg-muted/50 flex items-center justify-center font-bold text-xs text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                                                        {index + 1}
                                                    </div>
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="text-xs font-bold text-muted-foreground flex items-center gap-2 mt-1">
                                                            <Badge variant="secondary" className="px-2 py-0 h-4 text-xs font-bold rounded-sm">{item.productCode}</Badge>
                                                        </div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex justify-center items-center gap-2">
                                                    {isCompleted ? (
                                                        <Badge className="h-10 px-6 rounded-xl bg-muted text-muted-foreground border-0 font-bold text-lg">
                                                            {item.actualQty}
                                                        </Badge>
                                                    ) : (
                                                        <div className="relative group/input flex items-center">
                                                            <Input
                                                                ...
                                                                className={cn(
                                                                    "w-32 h-14 rounded-2xl text-center text-xl font-bold border-2 transition-all",
                                            ...
                                            <TableCell className="text-center">
                                                <div className="flex flex-col items-center gap-1">
                                                    {isBlind ? (
                                                        item.actualQty > 0 ? (
                                                            <Badge className="bg-primary/10 text-primary border-0 font-bold rounded-lg">
                                                                تم الجرد
                                                            </Badge>
                                                        ) : (
                                                            <Badge className="bg-muted text-muted-foreground border-0 font-bold rounded-lg">
                                                                بانتظار العد
                                                            </Badge>
                                                        )
                                                    ) : (
                                                        <>
                                                            {item.difference > 0 ? (
                                                                <Badge className="bg-success/10 text-success border-0 font-bold rounded-lg">
                                                                    زيادة {item.difference}+
                                                                </Badge>
                                                            ) : item.difference < 0 ? (
                                                                <Badge className="bg-destructive/10 text-destructive border-0 font-bold rounded-lg">
                                                                    عجز {item.difference}
                                                                </Badge>
                                                            ) : (
                                                                <Badge className="bg-muted text-muted-foreground border-0 font-bold rounded-lg">
